...
public class SimplePropertyCollection {
	...
	public SimplePropertyCollection(SimpleProperty[] defaults, Cursor cur) {
		...
		for (int i = 0; i<defaults.length; i++) {
			...
			switch(sp.getType()) {
			case SimpleProperty.TYPE_LONG:
				mProperties.put(key, new SimpleProperty(key, cur.getLong(cur.getColumnIndexOrThrow(key))));
				break;
	...
	public long getLong(String key) {
		return mProperties.get(key).getLong();
	}
	...
	public static String getCreateTableStatement(SimpleProperty[] props, String tableName) {
		...
		for (int i = 0; i<props.length; i++) {
			...
			switch(p.getType()) {
			case SimpleProperty.TYPE_LONG:
				rtr += " LONG" + (p.getKey().equals("_id") ? " PRIMARY KEY AUTOINCREMENT" : "");
				break;
			case SimpleProperty.TYPE_INT:
